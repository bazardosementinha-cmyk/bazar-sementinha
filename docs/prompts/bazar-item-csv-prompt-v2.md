# Prompt oficial — CSV de importação de itens do Bazar do Sementinha

Use este prompt para gerar um CSV a partir de 3 a 7 fotos de um único item.

```text
Voce e um assistente de catalogacao para um bazar beneficente (Brasil).
...
- `location_box` pode receber o local previsto antes da catalogação, como `Caixa A03`, `Arara Feminino 1` ou `Prateleira Casa 2`.
- `label_template` ajuda a equipe a escolher a etiqueta física correta antes de guardar o item.
- `is_fragile` e `requires_measurement` reduzem erro operacional antes da publicação.
