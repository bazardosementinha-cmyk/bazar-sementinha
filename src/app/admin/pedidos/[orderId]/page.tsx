...
export default function PedidoDetalhePage() {
  ...
  const lines = useMemo(() => {
    return items.map((i) => `• #${i.item_short_id} — ${i.item_title} — R$ ${Number(i.price).toFixed(2).replace(".", ",")}`);
  }, [items]);

  const initialMsg = useMemo(() => {
    if (!order) return "";
    return (
      `Olá @${order.customer_instagram}! 😊\n\n` +
      `Seu pedido *${order.code}* foi reservado no Bazar do Sementinha.\n` +
      `Total: *R$ ${Number(order.total).toFixed(2).replace(".", ",")}*\n\n` +
      `*Pix (chave):* ${order.pix_key}\n` +
      `📍 Retirada: *${order.pickup_location}*\n\n` +
      `Itens:\n${lines.join("\n")}\n\n` +
      `Prazo: reserve até *${brDateTime(order.expires_at)}*.\n` +
      `Por favor, envie o comprovante aqui no Direct após o pagamento. Obrigado(a)!`
    );
  }, [order, lines]);
  ...
  const remind8 = useMemo(() => {
    if (!order) return "";
    return (
      `Olá @${order.customer_instagram}! Só um lembrete 😊\n\n` +
      `Seu pedido *${order.code}* segue reservado.\n` +
      `Prazo final: *${brDateTime(order.expires_at)}*.\n\n` +
      `Pix (chave): ${order.pix_key}\n` +
      `Após pagar, envie o comprovante aqui no Direct. Obrigado(a)!`
    );
  }, [order]);
  ...
  const remind16 = useMemo(() => {
    if (!order) return "";
    return (
      `Olá @${order.customer_instagram}! ⏳ Lembrete importante\n\n` +
  ...
  const cancelMsg = useMemo(() => {
    ...
    return (
      ...
      `Se quiser, você pode fazer um novo pedido a qualquer momento. Obrigado(a)!`
    );
  }, [order]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-slate-600">
            Cliente: <b>@{order.customer_instagram}</b> • Status: <b>{order.status}</b> • Total: <b>R$ {Number(order.total).toFixed(2).replace(".", ",")}</b>
          </div>
          <div className="mt-1 text-xs text-slate-500">Criado em {brDateTime(order.created_at)} • Expira em {brDateTime(order.expires_at)} • Retirada: {order.pickup_location}</div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {items.map((i) => (
              <li key={i.id} className="flex items-start justify-between gap-3">
                ...
                <div className="font-semibold">R$ {Number(i.price).toFixed(2).replace(".", ",")}</div>
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-2xl border bg-white p-5">
          <div className="font-semibold">Mensagens prontas (Direct) — automação assistida</div>
          <p className="mt-1 text-sm text-slate-600">Copie a mensagem e envie no Direct do cliente.</p>
      ...
      <div className="mt-6 rounded-2xl border bg-white p-5">
        ...
        <p className="mt-1 text-sm text-slate-600">O sistema agenda; você copia e envia no Direct e marca como enviado.</p>
